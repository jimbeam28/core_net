...
mod tests {
    ...
    fn test_error_display() {
        let err = CoreError::BufferOverflow;
        assert!(err.to_string().contains("Buffer溢出"));

        let err = CoreError::InvalidLength { expected: 10, actual: 5 };
        assert!(err.to_string().contains("预期10"));
        assert!(err.to_string().contains("实际5"));
    }

    #[test]
    fn test_error_constructors() {
        let err = CoreError::parse_error("test error");
        assert!(matches!(err, CoreError::ParseError(_)));

        let err = CoreError::invalid_protocol("TCP");
        assert!(matches!(err, CoreError::InvalidProtocol(_)));

        let err = CoreError::unsupported_protocol("SCTP");
    ...
    fn test_result_type() {
        ...
        fn returns_error() -> Result<u32> {
            Err(CoreError::InvalidPacket("test".to_string()))
        }
